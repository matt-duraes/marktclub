...
trait TextoTrait
{
    ...
    {
        $this->_montarTextoJuridico();
        $this->_montarTextoDesconto();
        $this->_montarTextoVoucher();
        $this->_montarTextoValidar();
    }

    private function _montarTextoJuridico()
    {
        ...
        if ($this->Construtor->contato_telefone->valido()) {
            $this->texto_juridico .= '<br>Telefone: ' . $this->Construtor->contato_telefone->telefone() . ' (Apenas telefone fixo)';
        }
        if ($this->Construtor->contato_whatsapp->valido()) {
            $this->texto_juridico .= '<br>Whatsapp: ' . $this->Construtor->contato_whatsapp->telefone() . ' (apenas WhatsApp)';
        }
        if ($this->Construtor->contato_email->valido()) {
            $this->texto_juridico .= '<br>E-mail: ' . $this->Construtor->contato_email->email();
        }
    }
    private function _montarTextoDesconto()
    {
    ...
    private function _montarTextoVoucher()
    {
    ...
    private function _montarTextoValidar()
    {
