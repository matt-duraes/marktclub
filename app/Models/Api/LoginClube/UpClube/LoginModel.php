...
class LoginModel extends LoginPadraoModel
{
    ...
    public function __construct(
        ...
        $this->linkAutenticacao = env('UP_CLUBE_LINK_AUTENTICACAO', '');
    ...
    protected function buscarUsuarioPeloLoginSenha(): void
    {
        ...
        if (
            $status !== 200
            || (!is_object($dado) || !object_key_exists('status', $dado) || $dado->status !== 'Success')
        ) {
    ...
    private function buscarUsuarioNaBase(): void
    {
        ...
        $Usuario->email_pessoal = new Email($this->dado->data->user->contact->email);
        ...
        if ($Usuario->acao == SalvarAtualizarModel::CADASTRAR_USUARIO) {
            mensagemErro(
                ...
                dado: [
                    'cadastro' => 'sim',
                    'dado'     => [
                        'Email' => $this->dado->data->user->contact->email,
