...
trait PermissaoTrait {
    ...
    )
    {
        ...
        if($index) {
            array_merge(
                $lista,
                $this->montarIndicePermissao('index', $indice, $scopePadrao, $scope)
            );
        }
        return [
            'titulo' => $titulo,
    ...
    private function montarIndicePermissao(string $acao, string $indice, array $scopePadrao, string $scope)
    {
        $nomePermissao = [
            'index' => ['listar', self::TITULO_LISTAR],
        ];

        $scopeTemp = [$scope . ':' . $nomePermissao[$acao][0]];
        return $lista[$indice . '_' . $acao] = [
            'titulo' => $nomePermissao[$acao][1],
            'scope' => array_key_exists($acao, $scopePadrao) ? array_merge($scopePadrao[$acao], $scopeTemp) : $scopeTemp
        ];
    }
    // [
    ...
    private function setarPropriedadePermissao()
    {
        $this->PERMISSAO = [
            ...
            'comercial_indicacao' => $this->montarArrayPermissao(
                titulo: 'Comercial Indicação',
                indice: 'comercial_indicacao',
                scope: 'comercial-empresa',
                index: true,
                add: true,
                visualizar: true,
            ),
