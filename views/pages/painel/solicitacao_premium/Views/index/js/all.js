...
window.addEventListener('load', () => {
    ...
    const botaoCopiar = document.querySelector('#botao_copiar_geral');
    ...
    */
    if (botaoCopiar) {
        botaoCopiar.addEventListener('click', () => {
            copiarRegistroVisivel();
        });
    }
    const copiarRegistroVisivel = () => {
        let retorno = '';
        const lista = document.querySelectorAll('#bloco_app_titulo, #bloco_app_lista .bloco_lista .lista.geral');
        lista.forEach(linha => {
            const td = linha.querySelectorAll('.td');

            td.forEach(item => {
                let texto = item.classList.contains('status') ? item.getAttribute('data-titulo') : item.innerText;
