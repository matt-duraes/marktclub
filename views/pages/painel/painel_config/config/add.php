...
$Painel->coluna(callback: function () use ($Painel) {
    $Painel->fieldset('Campos Obrigátorios de Usuário', function () use ($Painel) {
        ...
        }, todos: 'Marcar todos os campos', mais: 1);
    });
});
    ...
    $Painel->fieldset('Campos Permitidos', function () use ($Painel) {
        $Painel->fieldsetCheckbox(callback: function () use ($Painel) {
            foreach (Padrao::CAMPOS_PERMITIDOS as $app => $dado) {
                ...
                if (array_key_exists('geral', $dado['recursos']) && !empty($dado['recursos']['geral'])) {
                    $Painel->margem('10');
                ...
                if (array_key_exists('download', $dado['recursos']) && !empty($dado['recursos']['download'])) {
                    $Painel->margem('10');
