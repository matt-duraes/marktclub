...
Route
    ...
    // LOGIN
    ::controller(LoginController::class)::nome('login')::grupo(function () {
        Route::rotaNaoUnica()::action('bloquear')::view('/bloquear');

        Route::rotaNaoUnica()::nome('sair')::view('/sair');
    }, true)

    // PERFIL
    ::nome('perfil')::controller(PerfilController::class)::grupo(function () {
        ...
        Route::nome('senha')::rotaNaoUnica()::view('/perfil/senha');

        Route::nome('atualizar_senha')::rotaNaoUnica()::action('senha')::request(['hash_validacao', 'senha_atual', 'senha_nova', 'senha_repetir'])::post('/perfil/senha');
    ...
    // AGENDA
    ::nome('agenda')::controller(AgendaController::class)::grupo(function () {
        Route::nome('index')::rotaNaoUnica()::view('/agenda');
    ...
    // APP
    ::controller(AppController::class)::grupo(function () {
        Route::action('index')::request(['!pesquisa', '!filtro', '!ordem', '!pagina'])::view('/app/{app}');

        Route::action('ajax')::request('*')::post('/app/ajax/{app}');

        Route::action('visualizar')::request('*')::view('/app/visualizar/{app}/{uuid}');

        Route::action('status')::request(['id', 'app', 'status'])::post('/app/status');

        Route::action('add')::view('/app/add/{app}');

        Route::action('editar')::request('*')::view('/app/editar/{app}/{uuid}');
        ...
        Route::action('ordem')::request(['id', 'pagina', 'hash_validacao'])::post('/app/ordem/{app}');

        Route::action('redirecionar')::request(['url'])::view('/app/redirecionar');
    }, true)
    ...
    // NOTIFICACAO
    ::controller(PainelController\NotificacaoController::class)::grupo(function () {
        Route::nome('listar')::rotaNaoUnica()::request(['pagina'])::get('/notificacao');
        Route::nome('atualizar')::rotaNaoUnica()::request(['id'])::post('/notificacao/atualizar-visualizadas');
        Route::nome('abrir')::rotaNaoUnica()::view('/notificacao/{id}');
        Route::nome('visualizarTodas')::rotaNaoUnica()::get('/notificacao/visualizar-todas');
    });
